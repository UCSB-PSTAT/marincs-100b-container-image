            ...
            stages{
                ...
                stage('Test') {
                    steps {
                        ...
                        sh 'podman run -it --rm --pull=never localhost/$IMAGE_NAME /opt/conda/bin/gdalinfo --formats | grep -i "hdf5"'
                        ...
                        retry (12) {
                            sh 'sleep 5 && curl -v http://localhost:8888/rstudio?token=jenkinstest 2>&1 | grep -P "HTTP\\S+\\s[1-3][0-9][0-9]\\s+[\\w\\s]+\\s*$"'
                        }
